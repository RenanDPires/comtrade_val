...
def get_pg_engine() -> Engine:
    ...
    return create_engine(
        ...
        connect_args=dict(
            ...
            user=DB_USER, password=DB_PASS, passfile="NUL",
        ),
        pool_pre_ping=True,
        future=True,
    )
...
# =============================================================================
# Helpers (pré-ingestão) – seus originais
# =============================================================================
...
# =============================================================================
# Extrair datetime base do STEM – seu original
# =============================================================================
...
# =============================================================================
# Pré‑ingestão – seu original (reutilizado)
# =============================================================================
...
def scan_substation(sub_cfg: dict) -> pd.DataFrame:
    ...
    if not packages:
        return pd.DataFrame(
            columns=[
                "substation", "ied", "directory", "stem",
                "expected", "present", "missing", "zero_kb",
                "max_arrival_skew_s", "received_ts", "integrity_ok",
...
# =============================================================================
# Análise COMTRADE – seu original (reutilizado)
# =============================================================================
...
def load_runtime_config_from_db(engine: Engine) -> List[dict]:
    ...
    for r in ieds:
        ied_id = r["ied_id"]; sub_id = r["sub_id"]
...
def main():
    ...
    for sub in subs_cfg:
        ...
        if WRITE_CSV:
            (HERE / SCAN_CSV_TPL.format(sub=sub["id"])).write_text(
                df_scan.to_csv(index=False, encoding="utf-8"), encoding="utf-8"
            )
            
        # 1b) grava no Postgres
        write_scan_df_to_pg(df_scan, sub["id"], cfg_id, engine)

        # 2) Análise de digitais
        df_analysis = analyze_integral_packages(sub, df_scan)
        if WRITE_CSV:
            (HERE / ANALYSIS_CSV_TPL.format(sub=sub["id"])).write_text(
                df_analysis.to_csv(index=False, encoding="utf-8"), encoding="utf-8"
            )
        # 2b) grava no Postgres
        write_analysis_df_to_pg(df_analysis, sub["id"], engine, cfg_id=cfg_id)
